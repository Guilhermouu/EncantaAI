(function () {
  ...
  .enc-menu button:hover { 
    background: #0056b3; 
  }

  @media (max-width: 300px) { 
    #encantai-widget-root { width: 100%; max-width: 100%; } 
  }
  `;
  ...
  <button class="enc-toggle-btn" id="enc-toggle-btn">
    <img id="open-icon" class="icon" src="./assets/images/botao_popup.png" alt="Abrir popup">
    <img id="close-icon" class="icon hidden" src="./assets/images/botao_popup2.png" alt="Fechar popup">
  </button>
  `;
  ...
  async function carregarManequim() {
    ...
    if (!manequim) {
      // Redireciona se não houver manequim
  ...
  // Trocar roupa (salva por usuário)
...
function carregarMenu() {
  ...
  const userId = localStorage.getItem("userId");
  if (userId !== "18") {
    // Não mostra nada para outros usuários
